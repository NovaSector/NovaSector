...
/mob/living/carbon/received_stamina_damage(current_level, amount_actual, amount)
	. = ..()
	if((maxHealth - current_level) <= crit_threshold && stat != DEAD && !(HAS_TRAIT(src, TRAIT_NOSTAMCRIT))) // NOVA EDIT CHANGE - Robots - if((maxHealth - current_level) <= crit_threshold && stat != DEAD)
