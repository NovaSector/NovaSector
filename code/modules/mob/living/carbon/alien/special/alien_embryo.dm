...
/obj/item/organ/body_egg/alien_embryo/proc/on_poll_concluded(gib_on_success, mob/dead/observer/ghost)
	...
	if(gib_on_success)
		...
		//owner.investigate_log("has been gibbed by an alien larva.", INVESTIGATE_DEATHS) // bobaEDIT REMOVAL - ALIEN QOL - don't ever gib host.
		//owner.gib(DROP_ORGANS|DROP_BODYPARTS)
		// bobaEDIT ADDITION BEGIN - ALIEN QOL - You aren't getting gibbed but you aren't going to be having fun
		...
		// bobaEDIT ADDITION END - ALIEN QOL
	else
