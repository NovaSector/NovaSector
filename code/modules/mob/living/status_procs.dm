...
/mob/living/proc/Sleeping(amount, is_voluntary = FALSE) //Can't go below remaining duration // bobaEDIT: Enhanced sleep - ORIGINAL: /mob/living/proc/SetSleeping(amount) //Can't go below remaining duration
	...
	if(S)
		S.duration = max(world.time + amount, S.duration)
	else if(amount > 0 || amount == STATUS_EFFECT_PERMANENT) // bobaEDIT CHANGE: Enhanced sleep - ORIGINAL: else if(amount > 0)
		S = apply_status_effect(/datum/status_effect/incapacitating/sleeping, amount, is_voluntary) // bobaEDIT: Enhanced sleep - ORIGINAL: S = apply_status_effect(/datum/status_effect/incapacitating/sleeping, amount)
	return S
...
/mob/living/proc/adjust_status_effects_on_shake_up()
	...
	// bobaEDIT BEGIN: Enhanced sleep - ORIGINAL: AdjustSleeping(-100)
