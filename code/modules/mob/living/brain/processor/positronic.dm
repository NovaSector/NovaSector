...
/obj/item/brain_processor/positronic/proc/stop_requesting_ghost()
	...
	if(!brainmob.ckey) // not an ideal spot for this but it works...
		visible_message(fail_message || span_notice("[src]'s lights fade away as [p_they()] slowly turn[p_s()] dormant...")) // NOVA EDIT CHANGE - ORIGINAL: visible_message(span_notice("[src]'s lights fade away as [p_they()] slowly turn[p_s()] dormant..."))
