...
/proc/is_banned_from(player_key, list/roles)
	...
	// bobaEDIT ADDITION BEGIN - MULTISERVER
	...
	// bobaEDIT ADDITION END - MULTISERVER
	var/sql_roles
	if(islist(roles))
...
/proc/is_banned_from_with_details(player_ckey, player_ip, player_cid, role)
	if(!player_ckey && !player_ip && !player_cid)
		return
	var/ssqlname = CONFIG_GET(string/serversqlname)   // bobaEDIT ADDITION BEGIN - MULTISERVER
	var/server_check
	if(CONFIG_GET(flag/respect_global_bans))
		server_check = "(server_name = '[ssqlname]' OR global_ban = '1')"
	else
		server_check = "server_name = '[ssqlname]'"  // bobaEDIT ADDITION END - MULTISERVER
	var/datum/db_query/query_check_ban = SSdbcore.NewQuery(/* NOVA EDIT CHANGE - MULTISERVER */{"
		SELECT
			id,
			bantime,
...
/datum/admins/proc/ban_panel(player_key, player_ip, player_cid, role, duration = 1440, applies_to_admins, reason, edit_id, page, admin_key, global_ban = TRUE) // bobaEDIT CHANGE - MULTISERVER
	...
	if(edit_id)
		...
		var/list/long_job_lists = list(
			...
			"Antagonist Positions" = list(
				...
				ROLE_TRAITOR,
				ROLE_WIZARD,
				ROLE_BORER, // bobaEDIT ADDITION
				ROLE_ASSAULT_OPERATIVE, // bobaEDIT ADDITION
				ROLE_VOIDWALKER,
			),
			"Nova Ban Options" = list(
				...
				BAN_ANTAGONIST,
				BAN_OPFOR,
				BAN_LOOC, // bobaEDIT ADDITION - LOOC muting again
			), // bobaEDIT ADDITION - EXTRA_BANS
		)
...
/datum/admins/proc/ban_parse_href(list/href_list)
	...
	var/global_ban = FALSE // bobaEDIT ADDITION - MULTISERVER
	var/duration
	var/interval
	var/severity
	var/reason
	...
	var/old_cid
	var/old_applies
	var/old_globalban // bobaEDIT ADDITION - MULTISERVER
	var/page
	var/admin_key
	...
	switch(href_list["radioservban"])  // bobaEDIT ADDITION BEGIN - MULTISERVER
		if("local")
			global_ban = FALSE
		if("global")
			global_ban = TRUE  // bobaEDIT ADDITION END - MULTISERVER
	...
	if(href_list["editid"])
		...
		old_globalban = href_list["old_globalban"]  // bobaEDIT ADDITION - MULTISERVER
		page = href_list["page"]
		admin_key = href_list["adminkey"]
		if(player_key != old_key)
			changes += list("Key" = "[old_key] to [player_key]")
		if(global_ban != old_globalban) // bobaEDIT ADDITION - MULTISERVER
			changes += list("Ban Location" = "[old_globalban] to [global_ban]") // bobaEDIT ADDITION - MULTISERVER
		if(player_ip != old_ip)
	...
	if(error_state.len)
		...
		return
	if(edit_id)
		edit_ban(edit_id, player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, reason, global_ban, mirror_edit, old_key, old_ip, old_cid, old_applies, page, admin_key, changes, roles_to_ban[1] == "Server") // bobaEDIT CHANGE - MULTISERVER
	else
		create_ban(player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, severity, reason, global_ban, roles_to_ban) // bobaEDIT CHANGE - MULTISERVER

/datum/admins/proc/create_ban(player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, severity, reason, global_ban, list/roles_to_ban) // bobaEDIT CHANGE - MULTISERVER
	if(!check_rights(R_BAN))
		return
	if(!SSdbcore.Connect())
		to_chat(usr, span_danger("Failed to establish database connection."), confidential = TRUE)
	...
	for(var/role in roles_to_ban)
		sql_ban += list(list(
			"server_name" = CONFIG_GET(string/serversqlname),// bobaEDIT CHANGE - MULTISERVER
			"server_ip" = world.internet_address || 0,
			"server_port" = world.port,
			"round_id" = GLOB.round_id,
			"role" = role,
			"global_ban" = global_ban, // bobaEDIT CHANGE - MULTISERVER
	...
	var/msg = "has created a [global_ban ? "global" : "local"] [isnull(duration) ? "permanent" : "temporary [time_message]"] [applies_to_admins ? "admin " : ""][is_server_ban ? "server ban" : "role ban from [roles_to_ban.len] roles"] for [target]." // bobaEDIT CHANGE - MULTISERVER
	...
	// bobaEDIT ADDITION BEGIN - EXTRA_BANS
	...
	//var/datum/admin_help/linked_ahelp_ticket = admin_ticket_log(player_ckey, "[kna] [msg]") // bobaEDIT ORIGINAL
	var/datum/admin_help/linked_ahelp_ticket = admin_ticket_log(player_ckey, "[kna] [msg]", FALSE) // bobaEDIT --  Player ticket viewing
...
/datum/admins/proc/edit_ban(ban_id, player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, reason, global_ban, mirror_edit, old_key, old_ip, old_cid, old_applies, admin_key, page, list/changes, is_server_ban) // bobaEDIT CHANGE - MULTISERVER
	if(!check_rights(R_BAN))
		return
	if(!SSdbcore.Connect())
		to_chat(usr, span_danger("Failed to establish database connection."), confidential = TRUE)
	...
	var/list/arguments = list(
		...
		"global_ban" = global_ban // bobaEDIT ADDITION - MULTISERVER
	)
	var/where
