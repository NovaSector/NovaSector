...
ADMIN_VERB(create_mapping_job_icons, R_DEBUG, "Generate job landmarks icons", "Generates job starting location landmarks.", ADMIN_CATEGORY_MAPPING)
	var/icon/final = icon()
	var/list/job_key_to_icon = list() // bobaEDIT ADDITION
	...
	for(var/job in subtypesof(/datum/job))
		...
		switch(JB.title)
			if(JOB_AI)
				job_key_to_icon["AI"] = icon('icons/mob/silicon/ai.dmi', "ai", SOUTH, 1) // bobaEDIT CHANGE - ORIGINAL: final.Insert(icon('icons/mob/silicon/ai.dmi', "ai", SOUTH, 1), "AI")
			if(JOB_CYBORG)
				job_key_to_icon["Cyborg"] = icon('icons/mob/silicon/robots.dmi', "robot", SOUTH, 1) // bobaEDIT CHANGE - ORIGINAL: final.Insert(icon('icons/mob/silicon/robots.dmi', "robot", SOUTH, 1), "Cyborg")
			else
				...
				job_key_to_icon[JB.title] = I // bobaEDIT CHANGE - ORIGINAL: final.Insert(I, JB.title)
	qdel(D)
