...
/datum/reagent/plantnutriment/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(SPT_PROB(tox_prob, seconds_per_tick))
		var/flipped_number = (affected_mob.mob_biotypes & MOB_PLANT) ? -1 : 1 //NOVA EDIT: Plant mobs will be healed instead
		if(affected_mob.adjustToxLoss(flipped_number, updating_health = FALSE, required_biotype = affected_biotype)) //NOVA EDIT: OLD VALUE: 1, NEW VALUE: flipped_number
			return UPDATE_MOB_HEALTH
