...
/datum/heretic_knowledge/proc/pre_research(mob/user, datum/antagonist/heretic/our_heretic)
	...
	if(is_final_knowledge && !our_heretic.unlimited_blades)
		var/choice = tgui_alert(user, "THIS WILL DISABLE BLADE BREAKING AND GIVE YOU A HERITICAL AURA! Are you sure you're ready to research this? The blade cap will also be removed.", "Get Final Spell?", list("Yes", "No")) //NOVA EDIT - Original: var/choice = tgui_alert(user, "THIS WILL DISABLE BLADE BREAKING, Are you ready to research this? The blade cap will also be removed.", "Get Final Spell?", list("Yes", "No"))
		if(choice != "Yes")
			return FALSE
	return TRUE
