...
/proc/random_bounty(category = 0)
	...
	while(!bounty_succeeded)
		...
		switch(bounty_num)
			...
			//NOVA ADDITION START - GHOST BOUNTIES #5738
			if(DS2_JOB_ENFORCER)
				chosen_type = pick(subtypesof(/datum/bounty/item/security))
			if(DS2_JOB_ENGINEER)
				chosen_type = pick(subtypesof(/datum/bounty/item/engineering))
			...
			if(DYNE_JOB_MINING)
				chosen_type = pick(subtypesof(/datum/bounty/item/mining) + subtypesof(/datum/bounty/item/sharedxenoarch))
			if(DYNE_JOB_SCIENCE)
				chosen_type = pick(subtypesof(/datum/bounty/item/interdyne_med) + subtypesof(/datum/bounty/item/interdyne_slime) + subtypesof(/datum/bounty/interdyne_pill/simple_pill) + subtypesof(/datum/bounty/interdyne_reagent/chemical_simple) + subtypesof(/datum/bounty/interdyne_reagent/chemical_complex))
			if(TARKON_JOB_CREW)
				chosen_type = pick(subtypesof(/datum/bounty/item/tarkon) + subtypesof(/datum/bounty/item/chef) + subtypesof(/datum/bounty/item/sharedxenoarch))
			if(TARKON_JOB_COMMAND)
				chosen_type = pick(subtypesof(/datum/bounty/item/tarkon) + subtypesof(/datum/bounty/item/sharedxenoarch))
			//NOVA ADDITION END - GHOST BOUNTIES #5738
