...
/obj/machinery/power/apc/proc/flicker_hacked_icon()
	if(opened != APC_COVER_CLOSED)
		return
	var/image/hacker_image = image(icon = icon, loc = src, icon_state = "apcemag", layer = FLOAT_LAYER) // NOVA EDIT CHANGE - ORIGINAL: var/image/hacker_image = image(icon = 'icons/obj/machines/wallmounts.dmi', loc = src, icon_state = "apcemag", layer = FLOAT_LAYER)
