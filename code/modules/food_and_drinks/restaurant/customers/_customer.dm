...
/datum/customer_data/moth/proc/get_wings(mob/living/basic/robot_customer/customer)
	...
	if (!LAZYACCESS(wings_chosen, customer_ref))
		LAZYSET(wings_chosen, customer_ref, pick(SSaccessories.sprite_accessories["wings"])) // bobaEDIT CHANGE - ORIGINAL: LAZYSET(wings_chosen, customer_ref, SSaccessories.moth_wings_list[pick(SSaccessories.moth_wings_list)])
