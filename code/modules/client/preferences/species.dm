...
/datum/preference/choiced/species/apply_to_human(mob/living/carbon/human/target, value, datum/preferences/prefs)
	target.set_species(value, FALSE, TRUE, TRUE, prefs?.features.Copy(), prefs?.mutant_bodyparts.Copy(), prefs?.body_markings.Copy()) // bobaEDIT CHANGE - Customization - ORIGINAL: target.set_species(value, icon_update = FALSE, pref_load = TRUE)
...
/datum/preference/choiced/species/compile_constant_data()
	...
	for (var/species_id in (get_selectable_species() + get_customizable_races())) //NOVA EDIT CHANGE
		...
		data[species_id]["nova_stars_only"] = species.nova_stars_only // bobaEDIT ADDITION - Veteran races
