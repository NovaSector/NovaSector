...
/obj/item/mod/module/proc/activate()
	if(!COOLDOWN_FINISHED(src, cooldown_timer))
		...
		return FALSE
	if(((!mod.active || mod.activating) && !(allow_flags & MODULE_ALLOW_INACTIVE)) || !mod.get_charge()) // NOVA EDIT ADDITION: INACTIVE USE
...
/obj/item/mod/module/proc/generate_worn_overlay(obj/item/source, mutable_appearance/standing)
	...
	var/custom_icon_file = mod.wearer?.dna?.species.get_custom_mod_module_icon() // NOVA EDIT ADDITION
	var/icon_file = custom_icon_file ? custom_icon_file : overlay_icon_file // NOVA EDIT ADDITION
	var/mutable_appearance/module_icon = mutable_appearance(icon_file, used_overlay, layer = standing.layer + 0.1) // NOVA EDIT, ORIGINAL: 	var/mutable_appearance/module_icon = mutable_appearance(overlay_icon_file, used_overlay, layer = standing.layer + 0.1)
	if(use_mod_colors)
