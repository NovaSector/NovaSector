...
/obj/effect/mapping_helpers/turn_off_lights_with_lightswitch/LateInitialize()
	...
	if(!needed_area.lightswitch && !needed_area.light_turned_off_at_spawn) // bobaEDIT CHANGE - Stops this when RNG turned the light off at roundstart - ORIGINAL: if(!needed_area.lightswitch)
