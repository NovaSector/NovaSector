...
/datum/unit_test/mecha_damage/Run()
	...
	qdel(dummy_laser.GetComponent(/datum/component/gun_safety)) // bobaEDIT - Safeties on guns make them impossible to shoot :)
	...
	qdel(dummy_gun.GetComponent(/datum/component/gun_safety)) // bobaEDIT - Safeties on guns make them impossible to shoot :)
