...
//Can't use allocate because of bug with certain datums
	...
	for(var/datum/design/surgery/path as anything in valid_subtypesof(/datum/design/surgery)) // NOVA EDIT CHANGE - ORIGINAL: for(var/datum/design/surgery/path as anything in subtypesof(/datum/design/surgery))
