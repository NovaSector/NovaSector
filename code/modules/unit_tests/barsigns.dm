...
/datum/unit_test/barsigns_icon
	var/list/blacklisted_sign_types = list(/datum/barsign/nova, /datum/barsign/nova/large) // bobaEDIT ADDITION - Modular barsigns
...
/datum/unit_test/barsigns_icon/Run()
	...
	barsign_icon_states += icon_states(NOVA_BARSIGN_FILE) // bobaEDIT ADDITION - Need to check modular barsigns
	barsign_icon_states += icon_states(NOVA_LARGE_BARSIGN_FILE) // bobaEDIT ADDITION - Need to check modular  barsigns

	// Check every datum real bar sign
	for(var/sign_type in (subtypesof(/datum/barsign) - /datum/barsign/hiddensigns))
		// bobaEDIT ADDITION BEGIN - MODULAR BARSIGNS
