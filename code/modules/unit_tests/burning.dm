...
/datum/unit_test/burning/Run()
	...
	// Light this baby up
	dummy.set_fire_stacks(9)
	dummy.ignite_mob()
	TEST_ASSERT(dummy.on_fire, "Dummy is not on fire despite having 9 fire stacks and being ignited.")
	...
	for(var/i in 1 to 5)
		...
		handler.process()
	TEST_ASSERT(dummy.fire_stacks < 9, "Dummy should have decayed firestacks, but did not. (Dummy stacks: [dummy.fire_stacks]).")
