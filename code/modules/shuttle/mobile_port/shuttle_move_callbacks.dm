...
/mob/living/lateShuttleMove(turf/oldT, list/movement_force, move_dir)
	...
	if(buckled && istype(get_area(src), /area/shuttle/arrival))
		...
		if (istype(oldT, /turf/open/space/transit) && !istype(buckled, /obj/vehicle/ridden/wheelchair)) // NOVA EDIT ADDITION - Ensures that the unbuckling only happens when its leaving hyperspace not entering
