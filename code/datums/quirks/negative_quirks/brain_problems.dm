	 ...
	 */
	// Halving the intensity of the quirk to make it less self torture.
	 /* - It should take 33 minutes 20 seconds to die from brain death due to a brain tumor now
	 * - It should take 3 minutes 20 seconds to take 10% (20 organ damage) as well
	 */
...
/datum/quirk/item_quirk/brainproblems/process(seconds_per_tick)
	quirk_holder.adjust_organ_loss(ORGAN_SLOT_BRAIN, 0.1 * seconds_per_tick) // this shouldn't be giving the user traumas every minute or so now
