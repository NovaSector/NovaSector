...
/datum/element/lube_walking/proc/lubricate(atom/movable/snail)
	...
	if(!istype(turf_standing_on))
		return FALSE
	turf_standing_on.MakeSlippery(wet_flags, min_wet_time = 1 SECOND) //NOVA EDIT CHANGE: Roundstart Snails - No more lube - ORIGINAL: turf_standing_on.MakeSlippery(wet_flags, min_wet_time = min_time_wet_for)
