...
/datum/ai_behavior/find_potential_targets/proc/failed_to_find_anyone(datum/ai_controller/controller, target_key, targeting_strategy_key, hiding_location_key)
	...
	// aggro_range = max(aggro_range, ROUND_UP(max(getviewsize(world.view)) / 2)) // bobaEDIT REMOVAL - This messes up the range of the stillcaps, will be possibly brought upstream as a result.
