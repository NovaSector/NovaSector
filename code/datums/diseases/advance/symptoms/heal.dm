...
/datum/symptom/heal/coma/CanHeal(datum/disease/advance/A)
	...
	switch(M.stat)
		...
		if(SOFT_CRIT)
			return power * 0.5
	if(M.getBruteLoss() + M.getFireLoss() >= 103 && !active_coma && !(HAS_TRAIT(M,TRAIT_NOSOFTCRIT))) // NOVA EDIT CHANGE - Adjusts this to remain 37.5% of total health(plus crit health). Because 70 is alot less health here then it is on tg. - ORIGINAL: if(M.getBruteLoss() + M.getFireLoss() >= 70 && !active_coma && !(HAS_TRAIT(M,TRAIT_NOSOFTCRIT)))
