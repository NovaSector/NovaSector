...
/datum/greyscale_config/New()
	...
	/* NOVA EDIT - Added recognition for the Skyrat GAGS folder - ORIGINAL:
	...
	*/ // ORIGINAL END - SKYART EDIT BEGIN:
	var/static/regex/skyrat_gags_regex = regex("(modular_nova/modules/GAGS/.*json_configs/)")
	if(findtext(string_json_config, "code/datums/greyscale/json_configs/") != 1 && skyrat_gags_regex.Find(string_json_config) != 1)
		stack_trace("All greyscale json configuration files should be located within '/greyscale/json_configs/' or 'modular_nova/modules/GAGS/json_configs/'.")
	// NOVA EDIT END
	if(!icon_file)
