...
/datum/chatmessage/proc/generate_image(text, atom/target, mob/owner, datum/language/language, list/extra_classes, lifespan)
	...
	var/voice_name = target.GetVoice() // NOVA EDIT ADDITION - Chat colors go by voice now
	if (!target.chat_color || target.chat_color_name != voice_name) // NOVA EDIT CHANGE - ORIGINAL: if (!target.chat_color || target.chat_color_name != name)
		target.chat_color = get_chat_color_string(voice_name) // NOVA EDIT CHANGE - ORIGINAL: target.chat_color = colorize_string(target.name)
		target.chat_color_darkened = get_chat_color_string(voice_name, darkened = TRUE) // NOVA EDIT CHANGE - ORIGINAL: target.chat_color_darkened = colorize_string(target.name, 0.85, 0.85)
