...
/datum/chatmessage/proc/generate_image(text, atom/target, mob/owner, datum/language/language, list/extra_classes, lifespan)
	...
	if (!target.chat_color || target.chat_color_name != chat_color_name_to_use)
		target.chat_color = get_chat_color_string(chat_color_name_to_use) // bobaEDIT CHANGE - ORIGINAL: target.chat_color = colorize_string(chat_color_name_to_use)
		target.chat_color_darkened = get_chat_color_string(chat_color_name_to_use, darkened = TRUE) // bobaEDIT CHANGE - ORIGINAL: target.chat_color_darkened = colorize_string(chat_color_name_to_use, 0.85, 0.85)
