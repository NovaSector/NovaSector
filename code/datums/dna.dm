...
/datum/dna/proc/copy_dna(datum/dna/new_dna, transfer_flags = COPY_DNA_SE|COPY_DNA_SPECIES)
	...
	if(new_dna.holder)
		...
		// new_dna.holder.set_species(species.type, icon_update = 0) // bobaEDIT REMOVAL
...
/datum/dna/proc/initialize_dna(newblood_type = random_human_blood_type(), create_mutation_blocks = TRUE, randomize_features = TRUE)
	...
	if(randomize_features)
		...
		features = species.randomize_features() | features // bobaEDIT CHANGE - Where applicable, replace features with the features generated by species/randomize_features() - Original: features["mcolor"] = "#[random_color()]"
		body_markings = species.get_random_body_markings(features) // bobaEDIT ADDITION

	mutant_bodyparts = species.get_mutant_bodyparts(features, existing_mutant_bodyparts = randomize_features ? list() : mutant_bodyparts) // bobaEDIT ADDITION
	update_dna_identity()
...
/mob/living/carbon/set_species(datum/species/mrace, icon_update = TRUE, pref_load = FALSE, replace_missing = TRUE, list/override_features, list/override_mutantparts, list/override_markings) // bobaEDIT CHANGE - ORIGINAL: /mob/living/carbon/set_species(datum/species/mrace, icon_update = TRUE, pref_load = FALSE, replace_missing = TRUE)
	...
	// bobaEDIT ADDITION START - BODYPARTS AND FEATURES
