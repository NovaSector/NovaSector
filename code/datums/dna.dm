...
/datum/dna/proc/copy_dna(datum/dna/new_dna, transfer_flags = COPY_DNA_SE|COPY_DNA_SPECIES)
	...
	if(new_dna.holder)
		...
		if(transfer_flags & COPY_DNA_SPECIES)
			new_dna.holder.set_species(species.type, icon_update = TRUE, pref_load = FALSE, override_features = features.Copy(), override_mutantparts = mutant_bodyparts.Copy(), override_markings = body_markings.Copy()) // NOVA EDIT CHANGE - ORIGINAL: new_dna.holder.set_species(species.type, icon_update = FALSE)
	else
