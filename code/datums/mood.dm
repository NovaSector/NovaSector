...
/datum/mood
	...
	/// What should we prefix the mood icon with?
	var/mood_prefix = ""
	/// What should we key the location of the mood screen element to?
	var/mood_location = ui_mood
	/// Should mood be fixed to neutral?
	var/forced_neutral = FALSE
	/// What icon path do we use? Used for setting the robot face sprites.
	var/icon_to_use = 'icons/hud/screen_gen.dmi'
...
/datum/mood/process(seconds_per_tick)
	// NOVA EDIT ADDITION START - Robots
	if(forced_neutral)
		set_sanity(SANITY_DISTURBED, override = TRUE)
	else
		switch(mood_level)
			if(MOOD_LEVEL_SAD4)
...
// NOVA EDIT ADDITION START - Robots
/datum/mood/proc/enable_robot()
	mood_prefix = "robot_"
	mood_location = ui_mood_robot
	icon_to_use = 'modular_nova/modules/robots/sprites/screen_gen.dmi'
	update_mood()

/datum/mood/proc/disable_robot()
	mood_prefix = ""
	mood_location = ui_mood
	icon_to_use = initial(icon_to_use)
	update_mood()

/datum/mood/proc/enable_forced_neutral()
	forced_neutral = TRUE
	update_mood()

/datum/mood/proc/disable_forced_neutral()
...
/datum/mood/proc/update_mood_icon()
	...
	// NOVA EDIT ADDITION START - Robots
	mood_screen_object.screen_loc = mood_location
	mood_screen_object.icon = icon_to_use
	// NOVA EDIT ADDITION END
	...
	// NOVA EDIT ADDITION START - Robots
	if(forced_neutral)
		mood_screen_object.color = "#696969" // nice
	else
		switch(sanity_level)
	...
	if (!conflicting_moodies.len) // there's no special icons, use the normal icon states
		//NOVA EDIT ADDITION BEGIN - ALEXITHYMIA and Robots
		if(HAS_TRAIT(mob_parent, TRAIT_MOOD_NOEXAMINE) && !forced_neutral)
			mood_screen_object.icon_state = "[mood_prefix]mood5"
			mood_screen_object.color = "#4b96c4"
			return
		mood_screen_object.icon_state = "[mood_prefix]mood[mood_level]"
		//NOVA EDIT ADDITION END
		return
	...
	for (var/datum/mood_event/conflicting_event as anything in conflicting_moodies)
		if (abs(conflicting_event.mood_change) == highest_absolute_mood)
			mood_screen_object.icon_state = "[mood_prefix][conflicting_event.special_screen_obj]" // NOVA EDIT CHANGE - Robots - mood_screen_object.icon_state = "[conflicting_event.special_screen_obj]"
			break
...
/datum/mood/proc/print_mood(mob/user)
	...
	if(!HAS_TRAIT(user, TRAIT_NOHUNGER)) // NOVA EDIT CHANGE - Robots - if(!HAS_TRAIT(src, TRAIT_NOHUNGER))
	...
	if (HAS_TRAIT(mob_parent, TRAIT_APATHETIC))
		...
		if(forced_neutral)
			msg += "[span_grey("I am simply not here.")]<br>"
		else
			switch(sanity)
		...
		if(forced_neutral)
			msg += "[span_grey("I have no strong feelings one way or the other.")]<br>"
		else
			switch(mood_level)
				if(MOOD_LEVEL_SAD4)
