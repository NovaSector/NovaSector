...
/proc/healthscan(mob/user, mob/living/target, mode = SCANNER_VERBOSE, advanced = FALSE, tochat = TRUE)
	...
	if(blood_id)
		...
		var/datum/blood_type/blood_type = carbontarget.dna.blood_type // NOVA EDIT CHANGE
		if(carbontarget.blood_volume <= BLOOD_VOLUME_SAFE && carbontarget.blood_volume > BLOOD_VOLUME_OKAY)
			...
			render_list += "<span class='alert ml-1'>Recommendation: [blood_type.restoration_chem::name] supplements or blood transfusion.</span><br>" // NOVA EDIT ADDITION
