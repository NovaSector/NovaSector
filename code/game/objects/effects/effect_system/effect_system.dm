...
/datum/effect_system/basic/proc/generate_effect()
	...
	RegisterSignal(loop, COMSIG_QDELETING, PROC_REF(decrement_total_effect)) // NOVA EDIT CHANGE - removes if(autocleanup)

/datum/effect_system/basic/proc/decrement_total_effect(datum/source)
	SIGNAL_HANDLER
	total_effects--
	if(autocleanup && total_effects == 0) // NOVA EDIT CHANGE - ORIGINAL: if(total_effects == 0)
