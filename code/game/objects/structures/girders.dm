...
/obj/structure/girder/attackby(obj/item/W, mob/user, params)
	var/skill_modifier = user.mind?.get_skill_modifier(/datum/skill/construction, SKILL_SPEED_MODIFIER) //NOVA EDIT: Construction Skill
	...
	if(istype(W, /obj/item/gun/energy/plasmacutter))
		...
		if(istype(W, /obj/item/stack/rods))
			...
			if(state == GIRDER_DISPLACED)
				...
				if(do_after(user, 2 SECONDS * skill_modifier, target = src))
					if(rod.get_amount() < amount)
						return
					rod.use(amount)
					var/obj/structure/falsewall/iron/FW = new (loc)
					user.mind?.adjust_experience(/datum/skill/construction, 2) //NOVA EDIT: Construction Skill
		...
		if(istype(sheets, /obj/item/stack/sheet/iron))
			...
			if(state == GIRDER_DISPLACED)
				if(sheets.get_amount() < amount)
					...
					return
				balloon_alert(user, "concealing entrance...")
				if(do_after(user, 20 * platingmodifier * skill_modifier, target = src))
					if(sheets.get_amount() < amount)
						return
					sheets.use(amount)
				...
				if(sheets.get_amount() < amount)
					...
					return
				balloon_alert(user, "adding plating...")
				if (do_after(user, 40 * platingmodifier * skill_modifier, target = src))
					if(sheets.get_amount() < amount)
						return
					sheets.use(amount)
					var/turf/T = get_turf(src)
		...
		if(istype(sheets, /obj/item/stack/sheet/plasteel))
			...
			if(state == GIRDER_DISPLACED)
				...
				if(do_after(user, 60 * platingmodifier * skill_modifier, target = src))
					if(sheets.get_amount() < amount)
						return
					sheets.use(amount)
