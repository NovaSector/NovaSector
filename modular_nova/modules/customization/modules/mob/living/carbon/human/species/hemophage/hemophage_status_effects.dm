...
/datum/status_effect/hemokinetic_regen/tick(seconds_between_ticks)
	...
	var/amount_healed = 0
	amount_healed += carbon_owner.adjustBruteLoss(-HEMOKINETIC_REGEN_HEALING * seconds_between_ticks, updating_health = FALSE)
	amount_healed += carbon_owner.adjustFireLoss(-HEMOKINETIC_REGEN_HEALING * seconds_between_ticks, updating_health = FALSE)
	if(amount_healed)
