/datum/species/mush/get_default_mutant_bodyparts()
	return list(
		...
		FEATURE_MUSH_CAP = list("Round", FALSE), // we don't want cap-less mushpeople
	)

/datum/species/mush/randomize_features()
	var/list/features = ..()
	features[FEATURE_MUSH_CAP] = pick(SSaccessories.feature_list[FEATURE_MUSH_CAP] - list("None")) // No tail-less monkeys. // No cap-less mushpeople.
	return features
