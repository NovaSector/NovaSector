...
/proc/reagent_process_flags_valid(mob/processor, datum/reagent/reagent)
	...
	if(human_processor.dna && human_processor.dna.species.reagent_flags)
		// Drugs only work on organic brains
		if(istype(reagent, /datum/reagent/drug) && (reagent.process_flags & REAGENT_ORGANIC))
			var/obj/item/organ/internal/brain/owner_brain = human_processor.get_organ_slot(ORGAN_SLOT_BRAIN)
		...
		if((reagent.process_flags & REAGENT_SYNTHETIC))
			if(processor_flags & PROCESS_SYNTHETIC)
				return TRUE
			var/obj/item/organ/internal/liver/owner_liver = human_processor.get_organ_slot(ORGAN_SLOT_LIVER)
			if(istype(owner_liver, /obj/item/organ/internal/liver/synth))
				return TRUE
