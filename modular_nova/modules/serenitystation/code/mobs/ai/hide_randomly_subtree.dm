...
/// This behavior causes the mob to go into hiding after a random duration
/// since the last time they went into hiding, or randomly come out of hiding.
/datum/bt_node/ai_behavior/random_hiding
...
/datum/bt_node/ai_behavior/random_hiding/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if(controller.blackboard[BB_HIDING_HIDDEN])
		...
		ai_toggle_hiding(controller, FALSE, cooldown_before_hiding_key)
		controller.set_blackboard_key(cooldown_before_hiding_key, world.time + rand(cooldown_minimum, cooldown_maximum) * START_HIDING_COOLDOWN_COEFFICIENT)
	...
	ai_toggle_hiding(controller, TRUE, cooldown_before_hiding_key)
	controller.set_blackboard_key(cooldown_before_stop_hiding_key, world.time + rand(cooldown_minimum, cooldown_maximum))
	return AI_BEHAVIOR_INSTANT | AI_BEHAVIOR_SUCCEEDED
