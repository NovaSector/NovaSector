...
ADMIN_VERB(toggle_solfed_signals, R_ADMIN, "Toggle Solfed Signals", "Toggles the ability for any solfed communication to be made", ADMIN_CATEGORY_GAME)
	...
	message_admins("[ADMIN_LOOKUPFLW(usr)] [GLOB.allow_solfed_signal? "allowed" : "disallowed"] all Sol Federation Communications to be sent")
	log_admin("[key_name(user)] [GLOB.allow_solfed_signal? "allowed" : "disallowed"] all Sol Federation Communications to be sent")
