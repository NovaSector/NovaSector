...
/area/ruin/interdyne_planetary_base/New()
	. = ..()
	RegisterSignal(SSdcs, COMSIG_GLOB_NEW_MACHINE, GLOBAL_PROC_REF(interdyne_machine))

/area/ruin/interdyne_planetary_base/Destroy()
	UnregisterSignal(SSdcs, COMSIG_GLOB_NEW_MACHINE)
	. = ..()

/proc/interdyne_machine(source, obj/machinery/target)
	SIGNAL_HANDLER
	...
	// Make sure we are on interdyne/ds2 base or shuttle.
	...
	// Stasis beds
		...
		stasis.announcement_channel = RADIO_CHANNEL_INTERDYNE
	// Brig timers
	...
	// Cryo tubes
		...
		cryo.broadcast_channel = RADIO_CHANNEL_INTERDYNE
