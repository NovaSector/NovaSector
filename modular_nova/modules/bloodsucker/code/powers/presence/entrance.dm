...
/datum/action/cooldown/vampire/targeted/entrance
	...
	ranged_mousepointer = 'modular_nova/modules/bloodsucker/icons/mouse_pointers/vampire_entrance.dmi'
...
/datum/action/cooldown/vampire/targeted/entrance/fire_targeted_power(atom/target_atom)
	...
	carbon_target.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/mesmerize.ogg', 50, FALSE, pressure_affected = FALSE)
...
/datum/status_effect/entranced/on_creation(mob/living/new_owner, set_duration)
	if(IS_FINITE(set_duration))
		...
		if(HAS_TRAIT(new_owner, TRAIT_UNCONVERTABLE))
			duration /= 2
	return ..()
