...
/datum/antagonist/vampire/proc/handle_blood()
	...
	if(current_vitae < VAMPIRE_LOW_BLOOD_WARNING && !low_blood_alerted)
		owner.current.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/bloodneed.ogg', 100, FALSE, pressure_affected = FALSE)
	...
	return HANDLE_BLOOD_HANDLED | HANDLE_BLOOD_NO_NUTRITION_DRAIN
...
/datum/antagonist/vampire/proc/final_death(skip_destruction = FALSE)
	...
	if(!skip_destruction)
		if(iscarbon(body))
			...
			playsound(owner.current, 'modular_nova/modules/bloodsucker/sound/burning_death.ogg', 100, TRUE)
		else
