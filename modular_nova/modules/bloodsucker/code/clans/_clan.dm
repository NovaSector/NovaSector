...
/datum/vampire_clan/New(datum/antagonist/vampire/owner_datum)
	...
	vampiredatum.owner.current.playsound_local(get_turf(vampiredatum.owner.current), 'modular_nova/modules/bloodsucker/sound/VampireAlert.ogg', 80, FALSE, pressure_affected = FALSE, use_reverb = FALSE)
	...
	log_uplink("[key_name(vampiredatum.owner.current)] has joined the [name].")
...
/datum/vampire_clan/proc/spend_rank(mob/living/carbon/carbon_vampire)
	...
	for(var/datum/discipline/discipline as anything in vampiredatum.owned_disciplines)	// We do owned_disciplines, not clan_disciplines. clan_disciplines is used to populate owned_disciplines.
		if(discipline.get_abilities_with_level("next"))
			...
			option.image = image(icon = 'modular_nova/modules/bloodsucker/icons/disciplines.dmi', icon_state = discipline.icon_state)
