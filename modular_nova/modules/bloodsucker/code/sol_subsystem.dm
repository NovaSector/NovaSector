	...
	switch(time_til_cycle)
		if(TIME_BLOODSUCKER_DAY_WARN)
			...
			warn_daylight(
				...
				vampire_warning_message = span_danger("Solar Flares will bombard the station with dangerous UV radiation in [TIME_BLOODSUCKER_DAY_WARN / 60] minutes. <b>Seek cover in an enclosed structure immediately.</b>"),
			)
		...
		if(NONE)
			...
			warn_daylight(
				...
				vampire_warning_message = span_userdanger("Solar flares bombard the station with deadly UV light! Stay in cover for the next [TIME_BLOODSUCKER_DAY / 60] minutes or risk Termination!"),
...
/datum/controller/subsystem/processing/sunlight/proc/warn_notify(mob/target, danger_level, message)
	...
	switch(danger_level)
		if(DANGER_LEVEL_FIRST_WARNING)
			target.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/griffin_3.ogg', 50, TRUE)
		if(DANGER_LEVEL_SECOND_WARNING)
			target.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/griffin_5.ogg', 50, TRUE)
