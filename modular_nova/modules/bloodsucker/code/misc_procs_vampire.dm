...
/datum/antagonist/vampire/proc/grant_power(datum/action/cooldown/vampire/power)
	...
	log_uplink("[key_name(owner.current)] has purchased: [power].")
...
/datum/antagonist/vampire/proc/break_masquerade(mob/admin)
	...
	owner.current.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/masquerade_violation.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/vampire/proc/rank_up(levels, ignore_reqs = FALSE, increase_goal = TRUE)
	...
	for(var/limb_slot, current_limb in affected_limbs)
		...
		if(QDELETED(limb) || !(limb_slot in GLOB.limb_zones))
			continue
...
/datum/antagonist/vampire/proc/adjust_humanity(count, silent = FALSE)
	...
	if(count > 0)
		...
		if(humanity < temp_humanity && !silent)
			owner.current.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/humanity_gain.ogg', 50, TRUE, pressure_affected = FALSE)
			if(power_given)
		...
		if(humanity > temp_humanity && !silent)
			owner.current.playsound_local(null, 'modular_nova/modules/bloodsucker/sound/humanity_loss.ogg', 50, TRUE, pressure_affected = FALSE)
			if(power_removed)
...
/datum/antagonist/vampire/proc/is_masq_watcher(mob/living/watcher, recursion = 1)
	...
	var/static/list/weirdo_antags
	if(isnull(weirdo_antags))
		weirdo_antags = list(
