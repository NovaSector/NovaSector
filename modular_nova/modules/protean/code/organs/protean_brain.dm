...
/obj/item/organ/brain/protean/proc/replace_limbs()
	...
	if(owner.dna.features[FEATURE_LEGS] == DIGITIGRADE_LEGS)
		var/obj/item/bodypart/old_left = owner.get_bodypart(BODY_ZONE_L_LEG)
		var/obj/item/bodypart/old_right = owner.get_bodypart(BODY_ZONE_R_LEG)
		...
		new_left.replace_limb(owner)
		new_right.replace_limb(owner)
		new_left.update_limb(is_creating = TRUE)
		new_right.update_limb(is_creating = TRUE)
		owner.update_body_parts()
		old_left?.moveToNullspace()
		old_right?.moveToNullspace()
		qdel(old_left)
		qdel(old_right)
	if(isnull(eyes))
