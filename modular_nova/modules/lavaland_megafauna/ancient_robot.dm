...
/mob/living/simple_animal/hostile/megafauna/ancient_robot/OpenFire()
	...
	if(prob(20)) //This attack is free, and can be combined with other attacks, so chance is low.
		single_laser()
...
/mob/living/simple_animal/hostile/megafauna/ancient_robot/proc/charge(atom/chargeat = target, delay = 5, chargepast = 2)
	...
	if(mode == BLUESPACE || (prob(13)))
		...
		do_teleport(src, target, precision= 7, asoundout= 'sound/effects/phasein.ogg') //Teleport within 7 tiles of the target
		new /obj/effect/temp_visual/bsg_kaboom(get_turf(src))
	...
	if(!T)
		return
	new /obj/effect/temp_visual/dragon_swoop/ancient_robot(T, beam)
	charging = TRUE
	revving_charge = TRUE
	DestroySurroundings()
	walk(src, 0)
...
/mob/living/simple_animal/hostile/megafauna/ancient_robot/bullet_act(obj/projectile/P, def_zone)
...
/mob/living/simple_animal/hostile/ancient_robot_leg/proc/beam_setup()
	leg_part = Beam(core.beam, "leg_connection", 'modular_nova/modules/lavaland_megafauna/icons/effects.dmi', time=INFINITY, maxdistance=INFINITY, beam_type=/obj/effect/ebeam, layer = 2 )

/mob/living/simple_animal/hostile/ancient_robot_leg/on_changed_z_level(old_z,new_z)
	..()
	update_z(new_z)
	if(leg_part)
		QDEL_NULL(leg_part)
...
/obj/effect/temp_visual/dragon_swoop/ancient_robot
	icon = 'modular_nova/modules/lavaland_megafauna/icons/96x96megafauna.dmi'
	icon_state = "warning"
	duration = 10

/obj/effect/temp_visual/dragon_swoop/ancient_robot/Initialize(mapload, target)
	. = ..()
	new /obj/effect/temp_visual/beam_target(get_turf(src), target)
	set_light(l_range = 4, l_power = 10,l_color = "#ee2e27")
...
/obj/effect/temp_visual/beam_target/Initialize(mapload, target)
	. = ..()
	charge = Beam(target, icon_state = "target_beam",icon = 'modular_nova/modules/lavaland_megafauna/icons/effects.dmi',  time=1.5 SECONDS, maxdistance=INFINITY, beam_type=/obj/effect/ebeam)
