...
/datum/artifact_effect/blood_regen/DoEffectTouch(mob/user)
	. = ..()
	if(!.)
		return
	to_chat(user, "<span class='notice'>Your body feels full, somehow.</span>")
	regen_target(user, 15)

/datum/artifact_effect/blood_regen/DoEffectAura()
	. = ..()
	if(!.)
		return
	var/turf/curr_turf = get_turf(holder)
	for(var/mob/living/receiver in range(range, curr_turf))
		to_chat(receiver, "<span class='notice'>Your chest feels warm.</span>")
		regen_target(receiver, 10)

/datum/artifact_effect/blood_regen/DoEffectPulse()
	. = ..()
	if(!.)
		return
	var/turf/curr_turf = get_turf(holder)
...
/datum/artifact_effect/blood_regen/DoEffectDestroy()
	var/turf/curr_turf = get_turf(holder)
...
/datum/artifact_effect/blood_drain/DoEffectTouch(mob/user)
	. = ..()
	if(!.)
		return
	to_chat(user, "<span class='warning'>Blood starts pouring out of your pores!</span>")
	suck(user, 50)
	return TRUE

/datum/artifact_effect/blood_drain/DoEffectAura()
	. = ..()
	if(!.)
		return
	var/turf/curr_turf = get_turf(holder)
	for(var/mob/living/receiver in range(range, curr_turf))
		to_chat(receiver, "<span class='warning'>Your nose bleeds.</span>")
		suck(receiver, 10)

/datum/artifact_effect/blood_drain/DoEffectPulse()
	. = ..()
	if(!.)
		return
	var/turf/curr_turf = get_turf(holder)
...
/datum/artifact_effect/blood_drain/DoEffectDestroy()
	var/turf/curr_turf = get_turf(holder)
	for(var/mob/living/receiver in range(7, curr_turf))
		to_chat(receiver, "<span class='warning'>You feel empty.</span>")
		suck(receiver, 75)
