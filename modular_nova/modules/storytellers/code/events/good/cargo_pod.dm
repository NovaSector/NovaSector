	...
	switch(good_level)
		...
		if(CARGO_GOOD_LEVEL_HIGH)  // High: Advanced resources + minor weapons (non-lethal/basic)
			...
			if(need_category == CARGO_NEED_MEDICAL)
				...
				possible_cargo += list(/obj/machinery/vending/snack = 1, /obj/item/toy/plush/human = 2)  // Vending or toys
				possible_cargo[/obj/item/toy/plush/human] *= base_weight_multiplier
			else if(need_category == CARGO_NEED_RESEARCH)
		...
		if(CARGO_GOOD_LEVEL_EXTREME)  // Extreme: Premium gear + advanced weapons
			...
			if(need_category == CARGO_NEED_MEDICAL)
				...
				possible_cargo += list(/obj/item/disk/design_disk/modkit_disc = 2, /obj/item/stock_parts/scanning_module/phasic = 1)
				possible_cargo[/obj/item/disk/design_disk/modkit_disc] *= base_weight_multiplier
			amount_of_pods = 3
