...
/datum/emote/living/subtle/run_emote(mob/user, params, type_override, intentional)
	...
	for(var/mob/receiver in viewers)
		if((emote_type & EMOTE_LEWD) && !pref_check_emote(receiver, preference = /datum/emote/living/lewd::pref_to_check))
			continue
...
/datum/emote/living/subtler/run_emote(mob/user, params, type_override, intentional)
	...
	if(istype(target, /mob))
		...
		if((emote_type & EMOTE_LEWD) && !pref_check_emote(target_mob, preference = /datum/emote/living/lewd::pref_to_check))
			return FALSE
		var/obj/effect/overlay/holo_pad_hologram/hologram = GLOB.hologram_impersonators[user]
		...
		if(hologram.Impersonation?.client)
			if((emote_type & EMOTE_LEWD) && !pref_check_emote(client = hologram.Impersonation.client, preference = /datum/emote/living/lewd::pref_to_check))
				return FALSE
