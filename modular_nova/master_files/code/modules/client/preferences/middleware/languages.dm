...
/datum/preference_middleware/languages/get_ui_data(mob/user)
	...
	for (var/language_path, language_instance in GLOB.language_datum_instances)
		...
		if(language.secret && species.language_prefs_whitelist && isnull(species.language_prefs_whitelist[language.type])) // For ghostrole species who are able to speak a secret language, e.g. ashwalkers, display it.
			continue
